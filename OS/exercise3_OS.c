    ...
    while (true) {
        if (xSemaphoreTake(press_semaphore,
        	...
        	if(!gpio_get_level(GPIO_SIDE_BUTTON)){
        		...
        		gpio_set_level(GPIO_DETECT_LED, 1);
        		}
        	else{
        		ESP_LOGI(TAG, "Not Press");
        		gpio_set_level(GPIO_DETECT_LED, 0);
        	}
...
void app_main(void) {
    ...
    lrs = gpio_set_pull_mode(GPIO_DETECT_LED, GPIO_FLOATING);
    if (lrs != ESP_OK) {
    	ESP_LOGE(TAG, "Error from gpio_set_pull_mode: %s", esp_err_to_name(lrs));
    	goto exit_on_fatal_error;
    }
    lrs = gpio_set_direction(GPIO_DETECT_LED, GPIO_MODE_OUTPUT);
    if (lrs != ESP_OK) {
    	ESP_LOGE(TAG, "Error from gpio_set_direction: %s", esp_err_to_name(lrs));
   		goto exit_on_fatal_error;
    }
   	uint32_t led_level = 0;
   	lrs = gpio_set_level(GPIO_DETECT_LED, led_level);
   	if (lrs != ESP_OK) {
   		ESP_LOGE(TAG, "Error from gpio_set_level: %s", esp_err_to_name(lrs));			goto exit_on_fatal_error;
    }
